...
class WindowAttention3D(nn.Module):
    ...
    def forward(self, q, kv=None, mask=None):
        ...
        if mask is not None:
            nW = mask.shape[0]
            attn = attn.view(B_ // nW, nW, self.num_heads, N1, N2) + mask.unsqueeze(1).unsqueeze(
                0
            )  # B, nW, nH, D1*Wh*Ww, D2*Wh*Ww
...
class VSTSREncoderTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        self.attn = WindowAttention3D(
            dim,
            num_frames_q=self.num_frames,
            num_frames_kv=self.num_frames,
...
class VSTSRDecoderTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        self.attn1 = WindowAttention3D(
            dim,
            num_frames_q=self.num_out_frames,
            num_frames_kv=self.num_out_frames,
            window_size=self.window_size,
            num_heads=num_heads,
            qkv_bias=qkv_bias,
            qk_scale=qk_scale,
    ...
    def forward(self, x, attn_kv, mask_matrix_q, mask_matrix_qkv):
        ...
        attn_kv_windows = attn_kv_windows.view(
            -1, D2 * window_size[0] * window_size[1], C
        )  # nW*B, D2*window_size*window_size, C

        # W-MSA/SW-MSA, # nW*B, D1*window_size*window_size, C
        attn_windows = self.attn2(x_windows, attn_kv_windows, mask=attn_mask_qkv)[0]
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, x, attn_kv):
        ...
        attn_mask_qkv = attn_mask_qkv.masked_fill(attn_mask_qkv != 0, float(-100.0)).masked_fill(
            attn_mask_qkv == 0, float(0.0)
        )
...
class InputProj(nn.Module):
    ...
    def __init__(self, in_channels=3, embed_dim=32, kernel_size=3, stride=1, norm_layer=None, act_layer=nn.LeakyReLU):
        super().__init__()

        self.embed_dim = embed_dim

        self.proj = nn.Sequential(
            nn.Conv2d(in_channels, embed_dim, kernel_size=kernel_size, stride=stride, padding=kernel_size // 2),
...
class Downsample(nn.Module):
    def __init__(self, in_chans, out_chans, type="conv"):
        super().__init__()
        if type == "conv":
            ...
            raise NotImplementedError(f"Downsample type {type} is not implemented.")
    ...
    def forward(self, x):
        ...
        x = x.view(-1, C, H, W)  # B*D, C, H, W
        out = self.conv(x)
        out = out.view(B, D, -1, H // 2, W // 2)  # B, D, C, H, W
        return out
